...
dns_labels = strats.from_regex(r"^[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])?$").map(strip)

# Image names (excluding digests for now)
# https://github.com/docker/distribution/blob/docker/1.13/reference/reference.go

host_components = strats.from_regex(
    r"^[a-zA-Z0-9]([a-zA-Z0-9-]{0,125}[a-zA-Z0-9])?$"
).map(strip)
port_numbers = strats.integers(min_value=1, max_value=32767)

hostnames_without_port = strats.just("localhost") | strats.builds(
    ".".join, strats.lists(elements=host_components, min_size=2, max_size=6)
)

hostnames_with_port = strats.builds(
    lambda h, p: str(h) + ":" + str(p), hostnames_without_port, port_numbers
)

hostnames = hostnames_without_port | hostnames_with_port

alphanumerics = strats.characters(
    min_codepoint=48, max_codepoint=122, whitelist_categories=["Ll", "Lu", "Nd"]
)
image_separators = (
    strats.just(".")
    | strats.just("_")
...
exact_image_names = strats.builds(
    "/".join, strats.lists(elements=image_components(), min_size=1, max_size=6)
)
# This is somewhat faster, if we don't care about having realistic
# image refs
sloppy_image_names = strats.text(
    string.ascii_letters + "-/_", min_size=1, max_size=255
).map(strip)
sloppy_image_names_with_host = strats.builds(
    lambda host, name: host + "/" + name, hostnames, sloppy_image_names
)
...
images_with_tag = strats.builds(
    lambda name, tag: name + ":" + tag, image_names, image_tags
)
...
workload_kinds = controller_kinds
all_kinds = controller_kinds + other_kinds
...
def manifests_equiv(man1, man2):
    try:
        if man1["kind"] != man2["kind"]:
            return False
        meta1, meta2 = man1["metadata"], man2["metadata"]
        if meta1.get("namespace", None) != meta2.get("namespace", None):
            return False
        ...
        if man1["kind"] in workload_kinds:
            return containers_equal(
                kubeyaml.containers(man1), kubeyaml.containers(man2)
            )
    except KeyError:
        return False
...
def controller_resources(draw):
    ...
    containers = list(
        map(lambda n: container(n, draw(images_with_tag)), container_names)
    )
    initcontainers = list(
        map(lambda n: container(n, draw(images_with_tag)), initcontainer_names)
    )
    podtemplate = {
        "template": {
            "spec": {"containers": containers, "initContainers": initcontainers}
        }
    }
...
image_only_values = (image_names | images_with_tag).map(
    lambda image: {"image": image, "_containers": [{kubeyaml.FHR_CONTAINER: image}]}
)
image_tag_values = strats.builds(
    lambda n, t: {
        "image": n,
        "tag": t,
        "_containers": [{kubeyaml.FHR_CONTAINER: "%s:%s" % (n, t)}],
    },
    image_names,
    image_tags,
)
image_registry_values = strats.builds(
    lambda r, n, t: {
        "registry": r,
        "image": "%s:%s" % (n, t),
        "_containers": [{kubeyaml.FHR_CONTAINER: "%s/%s:%s" % (r, n, t)}],
    },
    hostnames,
    exact_image_names,
    image_tags,
)
image_registry_tag_values = strats.builds(
    lambda r, n, t: {
        "registry": r,
        "image": n,
        "tag": t,
...
image_obj_values = strats.builds(
    lambda n, t: {
        "image": {"repository": n, "tag": t},
        "_containers": [{kubeyaml.FHR_CONTAINER: "%s:%s" % (n, t)}],
    },
    image_names,
    image_tags,
)
image_obj_repository_values = (image_names | images_with_tag).map(
    lambda image: {
        "image": {"repository": image},
        "_containers": [{kubeyaml.FHR_CONTAINER: image}],
    }
)
image_obj_registry_tag_values = strats.builds(
    lambda r, n, t: {
        "image": {"registry": r, "repository": n, "tag": t},
        "_containers": [{kubeyaml.FHR_CONTAINER: "%s/%s:%s" % (r, n, t)}],
    },
    hostnames,
    exact_image_names,
...
image_obj_registry_repository_values = strats.builds(
    lambda r, n, t: {
        "image": {"registry": r, "repository": "%s:%s" % (n, t)},
        "_containers": [{kubeyaml.FHR_CONTAINER: "%s/%s:%s" % (r, n, t)}],
    },
    hostnames,
    exact_image_names,
...
# One of the above
toplevel_image_values = (
    image_only_values
    | image_tag_values
    | image_registry_values
    | image_registry_tag_values
    | image_obj_values
    | image_obj_registry_tag_values
    | image_obj_registry_repository_values
)
# Some of the above, in fields
named_image_values = strats.dictionaries(
    keys=dns_labels, values=toplevel_image_values
).map(lift_containers)
# Combo of top-level image, and images in subfields
all_image_values = strats.builds(
    combine_containers, strats.just(None) | toplevel_image_values, named_image_values
)

values_noise = strats.deferred(
    lambda: strats.dictionaries(
        keys=dns_labels,
        values=values_noise
        | strats.integers()
        | strats.lists(values_noise)
        | strats.booleans()
        | strats.text(printable),
        max_size=3,
    )
)


workload_resources = controller_resources()
other_resources = strats.builds(resource_from_tuple, ids(other_kinds))

resources = workload_resources | other_resources
documents = resources | strats.builds(
    list_document, list_kinds, strats.lists(resources, max_size=6)
)
...
class Spec:
    ...
    def __repr__(self):
        return "Spec(kind=%s,name=%s,namespace=%s)" % (
            self.kind,
            self.name,
            self.namespace,
        )
...
def test_update_image_apply(docs, data):
    ...
    for d in docs:
        yaml.dump(d, original, explicit_start=True)
